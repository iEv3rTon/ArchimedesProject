...
async def autoscan(message):
    ...
    if m_pre_def and len(message.attachments) > 0 and message.attachments[0].filename[-4:].lower() == ".png":
...
async def print_welcome_message(guild):
    ...
    if c:
        await c.send("I'm {0}. If you need any help: ***dsc.gg/brasilop*** or ***@Ev3rTon***. \n "
                     "Supporting only PixelPlanet.fun, Canvaspixel.net, Pixmap.fun, Pixelcanvas.io. \nHosted on free service".format('Phi brother'))
        if len(prefixed) == 0:
